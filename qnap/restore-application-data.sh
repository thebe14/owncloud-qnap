...
BACKUP_PATH="/srv/owncloud-application-data/backups/"
RESTORE_PATH="/mnt/data/"
BACKUP_PREFIX="owncloud-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$OWNCLOUD_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$OWNCLOUD_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
